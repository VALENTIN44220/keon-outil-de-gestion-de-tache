-- Profils attendus : permission_profiles.name = 'Comptabilité' | 'Achat' (+ noms hybrides explicites).
-- Profil hybride : une seule validation peut poser compta ET achats en même temps.

DROP FUNCTION IF EXISTS public.classify_permission_profile_for_supplier_validation(text);
...
BEGIN
  ...
  -- Noms canoniques (insensible à la casse sur la valeur stockée).
  v_compta := v_norm = lower('Comptabilité');
  v_achats := v_norm = lower('Achat');

  -- Hybrides : ajuster la liste si vous créez d’autres profils combinés.
  IF v_norm IN (
