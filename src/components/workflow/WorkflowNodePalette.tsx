...
const paletteItems: NodePaletteItem[] = [
  ...
  // Standard Sub-processes (with built-in S1-S4 workflow)
  {
    type: 'sub_process_standard_direct',
    label: 'SP: Affectation directe',
    description: 'Tâches créées "À faire", notif demandeur + affecté',
    icon: <Layers className="h-4 w-4" />,
    color: 'bg-emerald-100 text-emerald-700 border-emerald-300',
    canAdd: true,
    category: 'standard_subprocess',
    defaultConfig: {
  ...
  {
    type: 'sub_process_standard_manager',
    label: 'SP: Affectation manager',
    description: 'Tâches créées "À affecter", manager affecte ensuite',
    icon: <UserPlus className="h-4 w-4" />,
    color: 'bg-amber-100 text-amber-700 border-amber-300',
    canAdd: true,
    category: 'standard_subprocess',
    defaultConfig: {
      assignment_mode: 'manager',
      initial_task_status: 'to_assign',
      notify_on_create: true,
      notify_on_status_change: true,
      notify_on_close: true,
      notify_requester: true,
  ...
  {
    type: 'sub_process_standard_validation1',
    label: 'SP: Validation 1 niveau',
    description: 'Validation manager avant clôture',
    icon: <ShieldCheck className="h-4 w-4" />,
    color: 'bg-cyan-100 text-cyan-700 border-cyan-300',
    canAdd: true,
    category: 'standard_subprocess',
    defaultConfig: {
      ...
      notify_assignee: true,
      validation_levels: 1,
      validation_1_approver_type: 'manager',
    },
  },
  {
    type: 'sub_process_standard_validation2',
    label: 'SP: Validation 2 niveaux',
    description: 'Validation demandeur puis manager',
    icon: <ShieldCheck className="h-4 w-4" />,
    color: 'bg-violet-100 text-violet-700 border-violet-300',
    canAdd: true,
    category: 'standard_subprocess',
    defaultConfig: {
      ...
      notify_assignee: true,
      validation_levels: 2,
      validation_1_approver_type: 'requester',
      validation_2_approver_type: 'manager',
    },
  },
];
...
export function WorkflowNodePalette({ onDragStart, disabled = false }: WorkflowNodePaletteProps) {
  ...
  const categories = [
    ...
    { id: 'standard_subprocess', label: 'Sous-processus (standard)' },
