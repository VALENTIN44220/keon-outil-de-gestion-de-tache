...
export function StepDetailsForm({ data, requestType, onDataChange, commonFieldsConfig }: StepDetailsFormProps) {
  ...
  // Pour les demandes 'process' : titre auto-genere (non editable).
  // Pour 'personal' et 'person' : l'utilisateur saisit son titre librement.
  const titleAutoGenerated = requestType === 'process';
  const cfg = {
    ...rawCfg,
    title: {
      ...rawCfg.title,
      editable: !titleAutoGenerated,
      title_pattern: rawCfg.title.title_pattern || '{process} - {date}',
    },
  };

  // Auto-generate title (process only)
  useEffect(() => {
    if (titleAutoGenerated && cfg.title.visible && cfg.title.title_pattern) {
  ...
  }, [titleAutoGenerated, cfg.title.title_pattern, data.processName, profile?.display_name]);
    ...
    <div className="space-y-6">
      ...
      <ScrollArea className="h-[450px] pr-4">
        <div className="space-y-5 pb-4">
          {/* Title : editable pour personal/person, auto-genere pour process */}
          {cfg.title.visible && (
            <div className="space-y-2">
              <Label htmlFor="title">
                Titre {!titleAutoGenerated && '*'}
                {titleAutoGenerated && (
              ...
              {titleAutoGenerated ? (
                ...
                <Input
                  ...
                  placeholder={isPersonal ? 'Ex. Préparer la réunion du lundi' : 'Décrivez la tâche à assigner...'}
                  required
                />
              )}
            </div>
